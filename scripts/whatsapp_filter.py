...
def extract_messages(input_file, output_file, sender):
    ...
    message_pattern = re.compile(r'^\[\d{1,2}/\d{1,2}/\d{2}, \d{1,2}:\d{2}:\d{2}.*?\]')
...
if __name__ == '__main__':
    subject = 'Matthew Chou'
    root = 'messages/whatsapp'
        ...
        for filename in filenames:
            if filename.endswith('.txt'):
                ...
                output_file = os.path.join(
                    output_dir,
                    f"{os.path.splitext(filename)[0]}_{subject.replace(' ', '_').lower()}.txt"
                )
